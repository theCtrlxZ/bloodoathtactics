    ...
    *,
    ...
    for raw in directives:
        ...
        if kind in ("trait", "traits"):
            ...
            ally["traits"].extend(picks)
            for tr in picks:
                apply_stat_mods(ally["stats"], tr.get("stat_mod"))
                apply_skill_mods(ally["skills"], tr.get("skill_mod"))
            if len(picks) < count:
                ctx["warnings"].append(
                    f"[{source_label}] roll:{kind} requested {count} but got {len(picks)} "
                    f"(pool={len(pool)}, tag={req or 'ANY'}, uniqueness exhausted or no matches)"
...
def finalize_name(base: str, ally: Dict[str, Any]) -> str:
    tags = {t.lower() for t in (ally.get("tags") or set())}
    traits = ally.get("traits") or []
...
def generate_ally(
    ...
    *,
    ...
    if required_class_name:
        ...
        matches = [c for c in data.ally_classes if str(c.get("name", "")).strip().lower() == want]
        cls = matches[0] if matches else None

    if cls is None and required_class_tags:
        restricted = [c for c in data.ally_classes if required_class_tags.intersection(entry_tags(c))]
        cls = weighted_choice(restricted) if restricted else None

    if cls is None:
        cls = weighted_choice(data.ally_classes) or {"name": "Unknown"}

    bg = weighted_choice(data.ally_backgrounds) or {"name": "Unknown"}
    ...
    apply_stat_mods(ally["stats"], bg.get("stat_mod"))
    apply_skill_mods(ally["skills"], bg.get("skill_mod"))

    # Base name
    base_name = pick_unique_name(data.ally_names, used_names)

    # Step 4: random traits
    trait_count = determine_trait_count()
    if trait_count > 0:
        pool = _pool_traits(data, {t.lower() for t in ally["tags"]})
    ...
    replace_placeholder_weapons(ally, data=data, ctx=ctx, merchant_level=merchant_level)
...
def format_ally(c: Dict[str, Any]) -> str:
    lines: List[str] = []
...
def generate_allies(data: DataBundle, inputs: Dict[str, Any]) -> Tuple[str, str]:
    ...
    body = ("\n" + "-" * 40 + "\n").join(format_ally(c) for c in allies)
